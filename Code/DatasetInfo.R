...
for (id in 1:ld)
{
  ...
  numNA[id]<- numInst[id] - sum(complete.cases(w))
...
ddata<-data.frame(dataset=datasets,NumInst=numInst,NumAtt=numat,ClassProportion=descr,NAs = numNA)

Tabdat <- xtable(ddata, caption= "Caption text", digits=3)
print.xtableFtable(Tabdat)
