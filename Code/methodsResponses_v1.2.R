...
.lib<- c("ggplot2","plyr","dplyr","RWeka", "sampling","gbm", "rpart", "e1071","randomForest","foreach","MASS","mlbench",
         ...
         "ada", "caTools", "adabag","ipred", "bst", "randomForest", "RRF", "pls", "KODAMA","caret")
...
obtain_responses <- function(ds= "_Toy_/", seed = 288, controlExperiment = FALSE, CVfolds = 5, balanceTestFold = FALSE){
  ...
  if (controlExperiment){
    ...
    methods <- c("MajorityClass", "MinorityClass", 
                 "OptimalClass","PessimalClass",
  ...
  }#"RandomClass_A", "RandomClass_B", "RandomClass_C", "RandomClass_D", "RandomClass_E",
  ...
  # # # # # # # # # # # # # # # # # # # # # # # # # 
  # experiments with 2 different DS
  # nfolds<-1
  # normal experiment
  nfolds <- CVfolds
  # over/undeSampling test fold
  balTestFold <- balanceTestFold
  # # # # # # # # # # # # # # # # # # # # # # # # # 
  ...
  for (id in 1:length(datasets))
  {
    ...
    matrixDSoverS <- matrix(NA, nrow = (tam*10), ncol = length(methods))
    colnames(matrixDSoverS) <- methods
    lastInsert <-0
    
    exportCSV <- data.frame()
    ...
    for (ik in 1:nfolds)
    {
      ...
      if (nfolds > 1){
        ...
        if (balTestFold){
          print("---- BALANCING TEST FOLDS ----")
          
          test<-w[indiceStrat,]
          
          minC <- which.min(as.vector(table(test$Class))) # index of min Class
          minC_class <- dimnames(table(test$Class))[[1]][minC] #actual Min Class
          numc = nrow(test[test$Class==minC_class,])

          majC <- which.max(as.vector(table(test$Class)))
          majC_class <- dimnames(table(test$Class))[[1]][majC]
          numC = nrow(test[test$Class==majC_class,])
          
          # if(numC == numc){
          #   majC_class = majC_class <- dimnames(table(test$Class))[[1]][majC+1]
          # }
          
          
          testOverS <-test
          for (class2bal in unique(test$Class)){
            
            howMany <- nrow(test[test$Class==class2bal,])
            
            if ((class2bal != majC_class) & (numC != howMany)){
              
              if (howMany > 0){
                indices.class2Bal <- which(test$Class == class2bal)
                indices.Maj <- which(test$Class == majC_class)
                overSample <- test[c(sample(indices.class2Bal, replace = TRUE, numC-howMany)),] #50-50
                rownames(overSample) <- paste("1000",rownames(overSample),sep="")
                testOverS <- rbind(testOverS,overSample) #oversampling 
              }
            }
          }

            test <- testOverS
            exportCSV <- rbind(exportCSV,test)
            print(paste("Balancing: ",paste(as.vector(table(test$Class)))))
        }else{#no balance test fold
          test<-w[indiceStrat,]
        }
      ...
      for (im in 1:length(methods))
      {
        ...
        if (methods[msel] == "bagFDA_prune2") { 
          ...
          #ERROR <-  tryCatch(model <- train(train[,-ncol(train)], train[,ncol(train)], data = train, method = "bagFDA",tuneGrid=data.frame(degree = 2, nprune= 2),  trControl = trainControl(method="none")), error = function(e) {return(TRUE)})
        ...
        if (methods[msel] == "RandomClass_D") { 
          #preds <- sample(unique(datos$Class),nrow(test), replace=T)
          preds <- sample(test$Class) 
          
        }
        
        if (methods[msel] == "RandomClass_E") { 
          #preds <- sample(unique(datos$Class),nrow(test), replace=T)
          preds <- sample(test$Class) 
          
        }
        
        if (methods[msel] == "RandomClass_F") { 
          #preds <- sample(unique(datos$Class),nrow(test), replace=T)
          preds <- sample(test$Class) 
          
        }
        
        if (methods[msel] == "RandomClass_G") { 
          #preds <- sample(unique(datos$Class),nrow(test), replace=T)
          preds <- sample(test$Class) 
          
        }
        
        if (methods[msel] == "RandomClass_H") { 
          #preds <- sample(unique(datos$Class),nrow(test), replace=T)
          preds <- sample(test$Class) 
          
        }
        
        if (methods[msel] == "RandomClass_I") { 
          #preds <- sample(unique(datos$Class),nrow(test), replace=T)
          preds <- sample(test$Class) 
          
        }
        
        if (methods[msel] == "RandomClass_J") { 
          #preds <- sample(unique(datos$Class),nrow(test), replace=T)
          preds <- sample(test$Class) 
        ...
        if (nfolds > 1) {
          if (balTestFold){ # test oversampled
            #print(paste("---------Last Row inserted: ", lastInsert))
             #print(paste("---------msel: ",msel))
             #print(paste("---------",(lastInsert+1),":",(length(preds)+lastInsert)))
            
            for(test_ex in (lastInsert+1):(length(preds)+lastInsert)){
              matrixDSoverS[test_ex,msel] = as.character(preds[test_ex-lastInsert])==as.character(test[test_ex-lastInsert,nomParamEstudio])
            }
             #print(preds)
             #print(paste("-------- matrix[",test_ex,",",msel,"]: ", matrixDSoverS[test_ex,msel]))
        ...
        }else{ # 2 different datasets (test - train)
          for(test_ex in 1:length(preds)){
            matrixDS[test_ex,msel] = preds[test_ex]==test[test_ex,nomParamEstudio]
          }
        }
    ...
    if (balTestFold){
      ListDS_Results[[datasets[selected]]]<- matrixDSoverS[rowSums(is.na(matrixDSoverS))!=length(methods),]
      write.csv(exportCSV,paste(ds,"Exported_",id,".csv",sep=""),row.names = FALSE)
    }else{
